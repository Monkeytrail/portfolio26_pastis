...
- **Pastis design system**: `public/pastis.js` is a pre-built (minified) Lit-based web component bundle providing custom elements like `<pastis-nav>`, `<pastis-theme-toggle>`, `<pastis-button>`. It's loaded via `next/script` in `src/app/layout.tsx` and consumed from React components (e.g. `src/components/Nav.tsx`) by setting DOM properties on refs after `customElements.whenDefined(...)` resolves, and by listening for custom DOM events (e.g. `pastis-theme-change`). **The source of truth is the separate [pastis-design-system](https://github.com/Monkeytrail/pastis-design-system) repo**, installed as a git dependency (`package.json` → `pastis-design-system`, pinned to a tag) and copied into `public/pastis.js` by `scripts/sync-pastis.mjs`, which runs automatically via the `postinstall` npm script. `public/pastis.js` is gitignored and generated on every `npm install` — don't hand-edit it, and don't commit it. To pick up a design-system change: bump the tag in the `pastis-design-system` dependency string in `package.json`, then run `npm install`.
